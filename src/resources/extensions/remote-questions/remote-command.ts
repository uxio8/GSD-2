...
async function handleSetupSlack(ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (!isValidChannelId("slack", channelId)) {
    return void ctx.ui.notify("Invalid Slack channel ID format — expected 9-12 uppercase alphanumeric characters.", "error");
  }
...
async function handleSetupDiscord(ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (!isValidChannelId("discord", channelId)) {
...
function removeRemoteQuestionsConfig(): void {
  ...
  const frontmatter = frontmatterMatch[1].replace(/remote_questions:[\s\S]*?(?=\n[a-zA-Z_]|\n---|$)/, "").trim();
...
function maskEditorLine(line: string): string {
  ...
  while (index < line.length) {
    ...
    const ansiMatch = /^\x1b\[[0-9;]*m/.exec(line.slice(index));
    if (ansiMatch) {
      output += ansiMatch[0];
